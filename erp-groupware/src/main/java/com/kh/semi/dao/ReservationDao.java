...
import com.kh.semi.mapper.ReservationMapper;
...
public class ReservationDao {
	...
	public List<ReservationDto> selectList() {
		String sql = "SELECT * FROM reservation "
				+ " order by calDate asc,stime asc, room_id asc";
		return jdbcTemplate.query(sql, reservationMapper);
	}

	// 검색기능 추가된 리스트
	public List<ReservationDto> selectList(String column, String keyword) {
		// 로직 구현
		String sql = "select * from reservation where instr(#1, ?) > 0 "
				+ " order by calDate asc, STIME asc, room_id asc";
