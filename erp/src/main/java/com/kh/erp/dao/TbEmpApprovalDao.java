...
//	}
	...
	public Integer getVacationApproCount(String applicantId, int vacaNo) {
		String sql = "SELECT COALESCE(SUM(v.date_diff), 0) " +
                "FROM (" +
                "    SELECT DISTINCT v.vaca_No, " +
                "           v.vaca_Sdate, " +
                "           v.vaca_Edate, " +
                "           v.vaca_Edate - v.vaca_Sdate " +  // 날짜 차이 계산 (+1일 포함)
                "    FROM tb_VacaReq v " +
                "    INNER JOIN tb_Approval a ON v.applicantId = a.applicantId " +
                "    AND a.appro_yn = 'N' " +
